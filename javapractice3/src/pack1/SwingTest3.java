...
public class SwingTest3 extends JFrame implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(txtName.getText().equals("")) {
			...
			txtName.requestFocus();  //사용자 편의를 위해 다시 넘겨줌
			return;   //리터해주고 빠짐
		}
		if(txtAge.getText().equals("")) {
			lblResult.setText("나이를 입력하시오");
			txtAge.requestFocus();  //사용자 편의를 위해 다시 넘겨줌
			return;   //리터해주고 빠짐
	}
		int nai = 0;
		try {
			...
			txtAge.setText(null);   //깨끗하게 비워짐 ""
			txtAge.requestFocus();
			return;
		}
	
		//System.out.println(rdoM.isSelected() + " " + rdoF.isSelected());   //어떤게 선택되었는지 확인
		String gender = "";
		if(rdoM.isSelected()) {
			gender ="남";
		}else {
